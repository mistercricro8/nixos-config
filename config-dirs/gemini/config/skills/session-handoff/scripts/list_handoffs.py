...
Searches for handoff documents in .agents/handoffs/ and displays:
- Filename with date
...
def main():
    ...
    if not handoffs:
        print(f"No handoffs found in {project_path}/.agents/handoffs/")
        print("\nTo create a handoff, run: python create_handoff.py [task-slug]")
        return

    print(f"Found {len(handoffs)} handoff(s) in {project_path}/.agents/handoffs/\n")
    print("-" * 80)
