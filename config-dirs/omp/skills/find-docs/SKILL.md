...
> ALWAYS run commands using `nix run nixpkgs#bun -- x ctx7@latest`. Do not use `npx`, `bunx`, or `pnpx` directly.

```bash
nix run nixpkgs#bun -- x ctx7@latest library <name> "<query>"
```
...
nix run nixpkgs#bun -- x ctx7@latest docs <libraryId> "<query>"
```
...
```bash
nix run nixpkgs#bun -- x ctx7@latest library React "How to clean up useEffect with async operations"
nix run nixpkgs#bun -- x ctx7@latest library "Next.js" "How to set up app router with middleware"
nix run nixpkgs#bun -- x ctx7@latest library Prisma "How to define one-to-many relations with cascade delete"
```
...
nix run nixpkgs#bun -- x ctx7@latest docs /vercel/next.js "How to set up app router"

# Version-specific
nix run nixpkgs#bun -- x ctx7@latest docs /vercel/next.js/v14.3.0-canary.87 "How to set up app router"
```
...
```bash
nix run nixpkgs#bun -- x ctx7@latest docs /facebook/react "How to clean up useEffect with async operations"
nix run nixpkgs#bun -- x ctx7@latest docs /vercel/next.js "How to add authentication middleware to app router"
nix run nixpkgs#bun -- x ctx7@latest docs /prisma/prisma "How to define one-to-many relations with cascade delete"
```
...
nix run nixpkgs#bun -- x ctx7@latest login
```

## Error Handling

1. **Execution Error**: Ensure Nix is installed and flakes/command-runner access is functional if the command fails to invoke.
2. **Quota Error**: If a command fails with a quota error ("Monthly quota reached" or "quota exceeded"):
* Inform the user their Context7 quota is exhausted.
* Suggest they authenticate for higher limits: `nix run nixpkgs#bun -- x ctx7@latest login`
...
* Running bare `npx`, `bunx`, or `pnpx` instead of `nix run nixpkgs#bun -- x ctx7@latest`
* Library IDs require a `/` prefix — `/facebook/react` not `facebook/react`
* Always run `nix run nixpkgs#bun -- x ctx7@latest library` first — `docs react "hooks"` will fail without a valid ID
